    ...
    public static function run(array $url_data) {
        ...
        if (class_exists($class) && method_exists($class, $action)) {
            ...
            if (DEBUG_MODE) {
                ...
                file_put_contents(ROOT . 'tmp' . DS . 'logs' . DS . date("Y-m-d") .
                    '-errors.txt', "\xEF\xBB\xBF" . $text, FILE_APPEND);
    ...
    public static function auto_load($class_name) {
        ...
        if ($class_name) {
            if (!class_exists($class_name)) {
                ...
                } elseif (is_readable(CONTROLLERS_PATH . mb_strtolower($name) . '.php')) {
                    require_once CONTROLLERS_PATH . mb_strtolower($name) . '.php';
                } elseif (is_readable(MODELS_PATH . $class_name . '.php')) {
                    require_once MODELS_PATH . $class_name . '.php';
                } elseif (is_readable(ROOT . 'config' . DS . 'autoloader.php')) {
                    require ROOT . 'config' . DS . 'autoloader.php';

                    if (
                        isset($autoloaded_classes)
                        ...
                        if (is_readable($autoloaded_classes[$class_name]['path'])) {
                            ...
                            try {
                                echo shell_exec(
                                    "git clone " . $autoloaded_classes[$class_name]['download'] . ' '
