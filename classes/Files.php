...
trait Files
{
    ...
    protected function get_files(
        ...
        if(is_dir($path)) {
            if($recursive === true) {
                ...
                if($get_files === true) {
                    $result = $this->get_files_recursively($iter, $file_ext, $get_full_path);
                }
                else {
                ...
                if($get_files === true) {
                    $result = $this->get_files_not_recursively($iter, $file_ext);
                }
                else {
                    foreach ($iter as $file_info) {
                        $dir_name = $file_info->getFilename();
    ...
    * @return (array) $result
    */
    private function get_files_recursively(
     ...
     * @return (array) $result
     */
    private function get_files_not_recursively(DirectoryIterator $i, array $exts = [])
    {
       $result = [];
