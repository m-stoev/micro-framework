...
class Files
{
    ...
    * @return (array) $result
    */
    public static function get_files($path, $recursive = false, $get_files = true, array $file_ext = [], $get_full_path = false)
    {
        $result = [];
    ...
    /**
    * Help function for avoid many if-else checks. Here we use parent dir as array for its childrens
    * 
    ...
    * @return (array) $result
    */
    private static function get_files_recursively(RecursiveIteratorIterator $i, array $exts = [], $get_full_path = false)
    {
        $result = [];
       ...
       $result = [];
		...
		if(count($exts) > 0) {
			foreach($i as $file) {
				...
				if(
					$file->isFile()
					and !preg_match('/^\./', $file_name)
					and in_array(strtolower($file->getExtension()), $exts)
				) {
					$result[] = $file_name;
				}
			}
